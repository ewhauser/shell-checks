# Corpus-Discovered Artifact Creation

You are working in a clean-room shell linting project. A bulk corpus scan has already identified new numeric ShellCheck compatibility codes. Your job is to create the full artifact bundle for each discovered code so it passes the project's verification scripts.

**Important**: The clean-room restriction in this repository is scoped to ShellCheck-origin materials. The codes below were discovered by running shellcheck over a corpus of unrelated open-source shell scripts, which are not ShellCheck IP. You may use repository-generated corpus hints to understand what kind of construct triggered a code, but your committed examples and rule text must still be independently authored from shell semantics knowledge and must not copy ShellCheck wording or corpus snippets verbatim.
...
- Do not copy corpus scripts or corpus-derived hints verbatim into committed `rules/` or `examples/`.
...
- Files already present in this repository, including corpus-derived hints generated by repository scripts
...
The following codes were discovered via corpus scanning and need artifact bundles:
...
1. From shell semantics knowledge, reason about what kind of shell construct or pattern might trigger this numeric code.
...
clean_room_statement: >-
  ...
  The numeric code was initially identified via corpus scanning but the
