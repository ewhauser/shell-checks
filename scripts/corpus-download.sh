...
for repo in $REPOS; do
  # Block shellcheck repos
  case "$repo" in
    *shellcheck*) echo "BLOCKED: $repo (shellcheck-related)"; continue ;;
  esac
  ...
  if grep -q "repo: $repo" "$manifest" 2>/dev/null; then
  ...
  # Find shell scripts: .sh files + files with shell shebangs
  count=0
  ...
  # Also find files with shell shebangs but no .sh extension
...
if [ "$dry_run" = false ]; then
  ...
  echo ""
  echo "==> Done. $final_count total scripts in corpus/scripts/"
fi
